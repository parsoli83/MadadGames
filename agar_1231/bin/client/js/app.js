...
/***/function (module, exports, __webpack_require__) {
	...
	window.onload = function () {
		...
		var btn = document.getElementById('startButton'),
		    ...
		    btnb = document.getElementById("button-back");
	...
	function setupSocket(socket) {
		...
		socket.on('playerDied', function (data) {
			window.chat.addSystemLine('{GAME} - <b>' + (data.name.length < 1 ? 'An unnamed cell' : data.name) + '</b> خورده شد.');
		});

		socket.on('playerDisconnect', function (data) {
			window.chat.addSystemLine('{GAME} - <b>' + (data.name.length < 1 ? 'An unnamed cell' : data.name) + '</b> رفت.');
		});

		socket.on('playerJoin', function (data) {
			window.chat.addSystemLine('{GAME} - <b>' + (data.name.length < 1 ? 'An unnamed cell' : data.name) + '</b> پیوست.');
		});

		socket.on('leaderboard', function (data) {
			leaderboard = data.leaderboard;
			var status = '<span class="title">جدول</span>';
