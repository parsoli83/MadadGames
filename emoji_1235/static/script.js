...
new TimelineMax({onStart:populate})
		...
		.set('.foundTxt',	{left:0, top:30, width:1024, fontSize:35, textAlign:'center', textContent:'ایموجی های یکسان رو بگو', fontWeight:300, letterSpacing:0.25})
...
function populate() {
    ...
    matched = false;
		msTilHint = 5000;
		TweenMax.set(hintBtn, {autoAlpha:0, textContent:"راهنمایی؟", fontSize:25});
    ...
    if (matched) return; //prevent user from repeatedly clicking the matching pair
        ...
        matched = true;
				...
				new TimelineMax({onComplete:populate})
					.set('.foundTxt', {textContent:'پیدا شده: '+found, fontWeight:500}, 0)
...
function updateTime(){
		if (timeLeft>0){
			...
			timeInt = undefined;
			if (found==0) TweenMax.set('.foundTxt', {textContent:'پیدا شده: 0', fontWeight:500});
			TweenMax.set('.endTxt', {textContent:hint+" دوباره امتحان کنیم؟ "+hint, fontSize:35});
...
replayBtn.onclick = replayBtn.ontouchend = function(){
  new TimelineMax()			
			...
			.call(function(){ // Reset game...
				TweenMax.set('.foundTxt', {textContent:'پیدا شده: 0'});
